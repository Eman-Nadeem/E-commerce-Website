...
export const createCheckoutSession=async(req, res)=>{
  try{
    ...
    const lineItems=products.map(product=>{
      ...
      totalAmount+=amount*product.quantity //calculate total amount based on the quatity of one product
      return { //format stripe wants
        price_data:{
          currency: 'usd',
          product_data: {
            name: product.name,
    ...
    res.status(500).json({message: 'Error processing sucessful checkout'})
  }
}
